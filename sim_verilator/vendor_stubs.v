...
module altsyncram #(
    ...
    input clock0, input clocken0, input clocken2, input clocken3, input rden_a, input aclr0,
    ...
    integer _ai_;
    initial for (_ai_=0; _ai_<numwords_a; _ai_=_ai_+1) mem[_ai_]=0;
    assign q_a = q_a_reg; assign q_b = q_b_reg; assign eccstatus = 0;
    always @(posedge clock0) begin if(wren_a&&!addressstall_a) mem[address_a]<=data_a; q_a_reg<=mem[address_a]; end
    always @(posedge clock1) begin if(wren_b&&!addressstall_b) mem[address_b]<=data_b; q_b_reg<=mem[address_b]; end
endmodule
...
module MMCME2_BASE #(
    ...
    // ── Behavioral PLL: 从 50MHz 生成 125/200/50MHz（真实硬件 VCO=1000MHz）──
    // 用 #delay 自由时钟正确建模 MAC 125MHz 时钟域（async FIFO 需写读同频）。
    // 所有时钟在 t=0 同相（4ns 网格），匹配硬件相位锁定关系。

    reg clk_125m_r = 0;
    always #4 clk_125m_r = ~clk_125m_r;   // 125MHz (8ns 周期)

    reg clk_200m_r = 0;
    always #2.5 clk_200m_r = ~clk_200m_r; // 200MHz (5ns 周期)

    reg clk_50m_cpu_r = 0;
    always #10 clk_50m_cpu_r = ~clk_50m_cpu_r; // 50MHz (20ns 周期)

    assign CLKOUT0 = clk_125m_r;    // clk_125m (MAC 125MHz)
    assign CLKOUT1 = clk_200m_r;    // clk_200m
    assign CLKOUT2 = clk_125m_r;    // clk_125m_tx (近似)
    assign CLKOUT3 = clk_50m_cpu_r; // clk_50m_cpu (CPU 50MHz)
    assign CLKOUT4 = clk_125m_r;
    assign CLKOUT5 = clk_125m_r;
    assign CLKOUT6 = clk_125m_r;
    assign CLKFBOUT = clk_125m_r;
    assign CLKFBOUTB = 1'b0;
    assign CLKOUT0B  = 1'b0;
    assign CLKOUT1B  = 1'b0;
    assign CLKOUT2B  = 1'b0;
    ...
    initial begin
        locked_r = 1'b0;
        #1000 locked_r = 1'b1;  // ~1us PLL lock time
    end
    assign LOCKED = locked_r;
endmodule
